...
use rex_core::{
    Compile, Expr, ExprError, ExprF, GExpr, NamedExpr, SpannedExpr, SpannedNamedExpr, VarKind
};
use rex_parser::parser::{LitKind, NormalSugarExpr, SugarExpr};

impl Compile for NormalSugarExpr {
    type Output = NamedExpr;

    type Error = ExprError<Self, Self::Span>;

    type Span = Vec<usize>;

    fn run(self) -> Result<rex_core::Spanned<Self::Output, Self::Span>, Self::Error> {
        todo!()
    }
}
