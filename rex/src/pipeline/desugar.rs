...
impl Compile for Desugar {
    type Input = Vec<FixAst>;
    type Output = NamedDefs;
    // Why is this a Vec<usize>?
    type Error = ExprError<Vec<usize>>;

    // this should just take one FixAst and then resolve instead
...
pub fn extract_binder(expr: FixAst, loc: Vec<usize>) -> Result<FullBinder, ExprError<Vec<usize>>> {
    ...
    match expr.0 {
        ...
        _ => {
            return Err(ExprError::InvalidBinder(loc.clone()));
        }
    }
}
...
pub fn fold_ty(binder: FullBinder, ty: SpannedNamedExpr, loc: Vec<usize>) -> SpannedNamedExpr {
    binder
        .1
        .clone()
        .into_iter()
...
pub fn fold_val(binder: FullBinder, body: SpannedNamedExpr, loc: Vec<usize>) -> SpannedNamedExpr {
    binder
        .1
        .clone()
        .into_iter()
        ...
        Binding(items) => {
            ...
            match named.0.0 {
                Ann(items) => {
                    ...
                    let ty = fold_ty(binder.clone(), ty, loc.clone());
                    let val = fold_val(binder.clone(), val, loc.clone());
...
pub fn desugar(expr: FixAst, loc: Vec<usize>) -> SpannedNamedExpr {
    use Ast::*;
    let new_expr = match expr.0 {
        Error(_, e) => ExprF::Err(
            ExprError::Unknown,
            iter_with_loc(e, loc.clone())
                .map(|(x, loc)| Box::new(desugar(x, loc)))
                .collect(),
        ),
        Type => ExprF::Type,
        Unit => ExprF::Lambda {
        ...
        App(items) => {
            let mut items = iter_with_loc(items, loc.clone()).map(|(x, loc)| desugar(x, loc));
            let base = items.next().unwrap();

            return items.fold(base, |acc, item| {
                SpannedGExpr((
                    ExprF::App {
                        func: Box::new(acc.clone()),
                        arg: Box::new(item),
                    },
                    loc.clone(),
                ))
        ...
        Lambda(items) => {
            ...
            return items.fold(desugar(base.0, base.1), |acc, item| {
                ...
                match binding {
                    ...
                    Err(e) => SpannedGExpr((ExprF::Err(e, vec![]), loc.clone())),
                }
            });
        }
        ...
        Ann(_) => {
            let Ok(binding) = extract_binding(expr, loc.clone(), BindingPriority::ValNameTy) else {
                return SpannedGExpr((ExprF::Err(ExprError::Unknown, vec![]), loc.clone()));
            };

            return binding.val.unwrap_or(SpannedGExpr((
                ExprF::Err(ExprError::MissingValue(loc.clone()), vec![]),
                loc.clone(),
            )));
        }
        Binding(_) => {
        ...
        } // We do not have a module system just yet, just a list of definitions
        // Module(_deps, items) => {
    ...
    SpannedGExpr((new_expr, loc.clone()))
}
...
// replace definitions while also changing definitions at runtime
type Def = (String, SpannedNamedExpr);
type Defs = Vec<Def>;
pub fn replace_defs(expr: SpannedNamedExpr, defs: &Defs) -> SpannedNamedExpr {
    fn go(expr: SpannedNamedExpr, ctx: &mut Vec<String>, defs: &Defs) -> SpannedNamedExpr {
        let new = match expr.0.0 {
            ExprF::Var { idx } => match idx {
                VarKind::Named(s) => {
                    if ctx.contains(&s) {
                        ExprF::Var {
                            idx: VarKind::Named(s),
                        ...
                        match defs.iter().rposition(|(name, _)| *name == s) {
                            Some(pos) => {
                                return defs.get(pos).unwrap().1.clone();
                            }
                            None => {
                                // println!("could not find name: {s}");
                                ExprF::Var {
                                    idx: VarKind::Named(s),
                                }
                            }
            ...
            ExprF::App { func, arg } => {
                let func = Box::new(go(*func, ctx, defs));
                let arg = Box::new(go(*arg, ctx, defs));
                ExprF::App { func, arg }
            }
            ExprF::Err(e, items) => ExprF::Err(
                e,
                items
                    .into_iter()
                    .map(|x| Box::new(go(*x, ctx, defs)))
                    .collect(),
            ),
            ExprF::Lambda {
                name,
                ...
                let param_ty = Box::new(go(*param_ty, ctx, defs));
                let body = Box::new(go(*body, ctx, defs));
            ...
            ExprF::Pi {
                ...
                let param_ty = Box::new(go(*param_ty, ctx, defs));
                let ret_ty = Box::new(go(*ret_ty, ctx, defs));
        ...
        SpannedGExpr((new, expr.0.1))
    }
    go(expr, &mut Vec::new(), defs)
}

pub fn add_defs(items: Vec<Def>, defs: &mut Defs) {
    for item in items {
        let expr = replace_defs(item.1, defs);
        defs.push((item.0, expr));
    }
}

pub fn create_subs_prev_typed(
    items: Vec<Spanned<Result<TypedBinding, ExprError<Vec<usize>>>>>,
    loc: Vec<usize>,
) -> Result<Vec<Spanned<TypedBindingNoOption>>, ExprError<Vec<usize>>> {
    let items = items
        .into_iter()
        .map(|item| {
            let binding = item.0?;
        ...
        .collect::<Result<Vec<Spanned<TypedBindingNoOption>>, ExprError<Vec<usize>>>>()?;

    // let later fields refer to previous fields directly without self reference modules
    let mut new_items: Vec<(TypedBindingNoOption, Vec<usize>)> = Vec::new();
    for (item, span) in items.clone() {
        let tuple_ctx = new_items
            .clone()
            .into_iter()
            .map(|x| x.0)
            .filter_map(|x| Some((x.name.map(|x| x.0)?, x.val)));

        let mut defs = Vec::new();
        for item in tuple_ctx {
            defs.push((item.0, item.1));
        }

        let ty = replace_defs(item.ty, &mut defs);
        let val = replace_defs(item.val, &mut defs);
        new_items.push((
...
// A1
// later fields can refer to previous fields directly without self
// reference modules
pub fn create_tuple(
    // list of bindings
    items: Vec<Spanned<Result<TypedBinding, ExprError<Vec<usize>>>>>,
    ...
    let new_items = create_subs_prev_typed(items, loc.clone())?;

    // NOTE: we used to achieve the same by creating application expressions instead but this was
    // to heavy on performance
    //
    // creating a new list by folding instead of map because dependency on previous values
    // let items: Vec<Spanned<TypedBindingNoOption>> =
    //     items.clone().into_iter().fold(Vec::new(), |mut acc, item| {
    //         let binding = item.0;
    //
    //         // the current one depends on all previous values
    ...
    //                 let item = item.0;
    //
    //                 // the type is actually a lambda that gets instantly applied with the previous
    //                 // field
    //                 let ty: SpannedExpr<VarKind<String, usize>, VarKind<String, ()>> =
    //                     SpannedExpr((
    //                         ExprF::App {
    //                             func: Box::new(SpannedExpr((
    //                                 ExprF::Lambda {
    //                                     name: VarKind::Named(
    //                                         item.name.clone().expect("This should be fixed").0,
    //                                     ),
    ...
    let body = new_items.clone().into_iter().fold(Ok(f_var), |acc, item| {
        let acc = acc?;
        let binding = item.0;
